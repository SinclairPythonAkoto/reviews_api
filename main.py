...
community_review_put_args.add_argument(
    "door", type=str, help="A door number is required...", required=True
)
community_review_put_args.add_argument(
    "street", type=str, help="A street name is required...", required=True
)
community_review_put_args.add_argument(
    "location", type=str, help="Place a town or city here..."
)
community_review_put_args.add_argument(
    "postcode", type=str, help="A postocde is required...", required=True
)
community_review_put_args.add_argument(
    "rating", type=int, help="Please enter a value from 1 - 5", required=True
)
community_review_put_args.add_argument(
    "review", type=str, help="Place your review here..."
)
community_review_put_args.add_argument(
    "reviewee",
    type=str,
    help="Enter one of the following: 'tenant', 'neighbour' or 'visitor'",
    required=True,
)

community_review_update_args = reqparse.RequestParser()
community_review_update_args.add_argument(
    "rating", type=int, help="A rating is needed to update..."
)
community_review_update_args.add_argument(
    "review", type=str, help="A review is needed to update..."
)
community_review_update_args.add_argument(
    "reviewee", type=str, help="A reviewee is needed to update..."
)

create_review_put_args = reqparse.RequestParser()
create_review_put_args.add_argument("door", type=str, help="A door number is required...", required=True)
create_review_put_args.add_argument("street", type=str, help="A street name is required...", required=True)
create_review_put_args.add_argument("location", type=str, help="Place a town or city here...")
create_review_put_args.add_argument("postcode", type=str, help="A postocde is required...", required=True)
...
def abort_if_bad_request():
    abort(
        400,
        message="Oops, something went wrong with your request! Please check your details and try again...",
    )
...
def abort_if_incorrect_reviewee(reviewee):
    if reviewee not in ["tenant", "neighbour", "visitor"]:
        abort(
            400,
            message="Wrong reviewee type. Enter one of the following: 'tenant', 'neighbour' or 'visitor'",
        )
...
class CommunityReviewID(Resource):
    ...
    def put(self, review_id):
        ...
        map_response = requests.get(
            f"{OPENSTREETMAP_URL}&postalcode={postcode}&country=united kingdom"
        )
        ...
        if validate_postcode == False:
            ...
            if map_data:
                ...
                new_map_entry = Maps(
                    lon=longitude, lat=latitude, location=new_address_entry
                )
                ...
                response.headers[
                    "Custom-Header"
                ] = f"Review {review_id} successfully created using new address & map coordinates."
        ...
        # if door & postcode match - create review from existing address db (query db and link address id to new review foreign key)
            ...
            find_address = (
                ...
                .filter_by(door_num=door, postcode=postcode)
                .first()
            )
            # create a new review
            new_review_entry = Review(
            ...
            response.headers[
                "Custom-Header"
            ] = f"Review {review_id} successfully created using existing address."
            return make_response(response, 201)
    ...
    def put(self):
        ...
        if validate_postcode == False:
            ...
            if map_data:
                ...
                response.headers[
                    "Custom-Header"
                ] = "A new review was successfully created using a new address & map coordinates."
                return make_response(response, 201)
            else:
                abort_if_bad_request()
        # if postcode matches but diff door number - create new address, review, map
...
class FilterReviewByTenant(Resource):
    def get(self, tenant):
        tenant = tenant.lower()
        review_list = []
        find_review = db.session.query(Review).filter_by(type=tenant).first()
        if not find_review:
            abort(404, message="Could not find review.")
        get_reviews = db.session.query(Review).all()
        for review in get_reviews:
            if tenant == review.type:
                data = {
                    "review_id": review.id,
                    "status": 302,
                    "Address": {
                ...
                review_list.append(data)
        response = jsonify({"Filtered Reviews": review_list})
        response.headers["Custom-Header"] = f"Display all reviews with matching reviewee: {tenant}."
        return make_response(response, 302)
